...
class VulkanWindow : public Napi::ObjectWrap<VulkanWindow> {
  ...
  public:
    ...
    Napi::Value enterPointerLock(const Napi::CallbackInfo &info);
...
Napi::Object VulkanWindow::Initialize(Napi::Env env, Napi::Object exports) {
  ...
  Napi::Function func = DefineClass(env, "VulkanWindow", {
    ...
    InstanceMethod(
      "enterPointerLock",
      &VulkanWindow::enterPointerLock,
      napi_enumerable
    ),
    InstanceMethod(
      "exitPointerLock",
      &VulkanWindow::exitPointerLock,
      napi_enumerable
    ),
    InstanceMethod(
      "shouldClose",
  ...
  if (info.IsConstructCall()) {
    if (info[0].IsObject()) {
      ...
      if (obj.Has("resizable") && obj.Get("resizable").IsBoolean()) {
        glfwSetWindowAttrib(window, GLFW_RESIZABLE, obj.Get("resizable").As<Napi::Boolean>().Value() ? GLFW_TRUE : GLFW_FALSE);
      }
...
Napi::Value VulkanWindow::close(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  GLFWwindow* window = this->instance;
  if (!this->isClosed) VulkanWindow::onWindowClose(window);
  return env.Undefined();
}

Napi::Value VulkanWindow::enterPointerLock(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  GLFWwindow* window = this->instance;
  if (!this->isClosed) {
...
Napi::Value VulkanWindow::exitPointerLock(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  GLFWwindow* window = this->instance;
  if (!this->isClosed) {
...
Napi::Value VulkanWindow::createSurface(const Napi::CallbackInfo& info) {
  ...
  VkInstance* instance = (VkInstance*)resolveNapiValueAddress(info[0]);
  VkSurfaceKHR* surface = (VkSurfaceKHR*)resolveNapiValueAddress(info[1]);
